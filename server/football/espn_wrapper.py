...
class ESPNWrapper:
    ...
    def final_standings(self):
        ...
        standings = sorted(
            final_records, key=lambda i: (i["wins"], i["points_scored"]), reverse=True
        )

        printed_standings = [
            "{}. {} ({} - {}) {} points scored".format(
                count,
                team["name"],
                team["wins"],
                team["losses"],
                round(team["points_scored"], 2),
            )
    ...
    def simulate_season(self):
        ...
        for week in range(self.league.current_week, 13):
            ...
            for matchup in matchups:
                ...
                home_points = (
                    average_home_points + home_drift
                    if random.choice([1, 2]) == 1
                    else average_home_points - home_drift
                )
                ...
                away_points = (
                    average_away_points + away_drift
                    if random.choice([1, 2]) == 1
                    else average_away_points - away_drift
                )
    ...
    def get_average_points_scored(self, team=None):
        ...
        averages = [
            "%s - %s" % (i["name"], i["average"])
            for i in sorted(team_scores, key=lambda i: i["average"], reverse=True)
        ]
    ...
    def get_trophies(self, week=None):
        if not week:
            ...
            boxes = self.league.box_scores(week=week)
            for box in boxes:
                if not self.all_played(box.away_lineup) or not self.all_played(
                    box.home_lineup
                ):
        ...
        for i in matchups:
            ...
            if (
                i.away_score - i.home_score != 0
                and abs(i.away_score - i.home_score) < closest_score
            ):
        ...
        high_score_str = [
            "High score: %s racked up %.2f points" % (high_team_name, high_score)
        ]
        close_score_str = [
            "Fleece of the week: %s beat %s by %.2f points"
            % (close_winner, close_loser, closest_score)
        ]
        blowout_str = [
            "Wax of the week: %s blew out %s by %.2f points"
            % (ownerer_team_name, blown_out_team_name, biggest_blowout)
        ]

        star, bust = self.players_of_the_week(week)
        star_str = [
            "Thiqqy of the week: {} scored {} points ({} projected)".format(
                star.name, star.points, star.projected_points
            )
        ]
        bust_str = [
            "Pencil of the week: {} scored {} points ({} projected)".format(
                bust.name, bust.points, bust.projected_points
            )
        ]

        text = (
            [f"Week {week} Trophies:"]
            + high_score_str
            + low_score_str
            + close_score_str
            + blowout_str
    ...
    def get_projected_scoreboard(self, week=None):
        ...
        score = [
            ...
            % (
                ...
                self.get_projected_total(i.home_lineup),
                self.get_projected_total(i.away_lineup),
    ...
    def scoreboard(self, week=None):
        ...
        for matchup in scoreboard:
            scores += [
                "%s %.2f - %.2f %s"
                % (
                    matchup.home_team.team_abbrev,
                    matchup.home_score,
                    matchup.away_score,
                    matchup.away_team.team_abbrev,
                )
            ]
    ...
    def matchups(self, week=None):
        ...
        for matchup in scoreboard:
            scores += [
                "%s vs. %s"
                % (matchup.home_team.team_abbrev, matchup.away_team.team_abbrev)
            ]
    ...
    def standings(self):
        ...
        for team in self.league.standings():
            matchup = next(
                m for m in scoreboard if m.home_team == team or m.away_team == team
            )
        ...
        standings = sorted(
            current_standings,
            key=lambda i: (i["wins"], i["points_scored"]),
            reverse=True,
        )
